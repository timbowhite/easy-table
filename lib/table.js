...
Table.prototype.cell = function (col, obj, printer, length, color) {
    this.line[col] = {
        value:  obj,
        color: color
    };
...
Table.prototype.toString = function () {
    ...
    this.lines.forEach(function (line) {
        s += self._printLine(function (key, col) {
            var color = line[key].color || undefined,
                value = color ? line[key].value.toString()[color] : line[key].value; 
            return col.printer(value, col.length);
        });
    });
...
function LeftPadder (char) {
    ...
    return function (obj, length) {
        var ret = '';
        var s = String(obj);
        for (var i = 0; i < length - realLength(s); i++) {
            ret += char;
        }
        ret += s;
        return ret;
